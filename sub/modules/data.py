...
from sub.config import MONGO_URL

mongo_client = AsyncIOMotorClient(MONGO_URL)
mongodb = mongo_client.multysub
...
async def cek_owner(user_id):
    r = [jo async for jo in ownerdb.find({"user_id": user_id})]
    if r:
        return r
    else:
        return False
...
async def get_subs(user_id):
    r = [jo async for jo in subdb.find({"user_id": user_id})]
    if r:
        return r
    else:
        return False
...
async def get_user(user_id):
    r = [jo async for jo in broaddb.find({"user_id": user_id})]
    if r:
        return r
    else:
        return False
...
async def cek_admin(user_id):
    x = [jo async for jo in admindb.find({"user_id": user_id})]
    if x:
        return x
    else:
        return False
...
async def max_info(user_id):
    ...
    if not active:
        return 2
    return active["maxsub"]


async def get_start(user_id: int):
    msg = await msgdb.find_one({'user_id': user_id})
    if not msg:
        return None
    else:
        return msg['caption']
...
async def set_start(user_id: int, caption: str):
    cek = await get_start(user_id)
    if cek:
        ...
        await msgdb.insert_one({'user_id': user_id, 'caption': caption})


async def del_start(user_id: int):
    await msgdb.delete_one({'user_id': user_id})
